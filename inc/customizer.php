...
function muhtawaa_customize_register($wp_customize) {
    ...
    // قسم إعدادات المحتوى
    $wp_customize->add_section('muhtawaa_content', array(
        'title'    => __('إعدادات المحتوى', 'muhtawaa'),
        'priority' => 48,
    ));

    // إظهار ملخص المقال
    $wp_customize->add_setting('muhtawaa_show_summary', array(
        'default'           => true,
        'sanitize_callback' => 'wp_validate_boolean',
        'transport'         => 'postMessage',
    ));

    $wp_customize->add_control('muhtawaa_show_summary', array(
        'label'   => __('إظهار ملخص المقال', 'muhtawaa'),
        'section' => 'muhtawaa_content',
        'type'    => 'checkbox',
    ));

    // طول الملخص
    $wp_customize->add_setting('muhtawaa_summary_length', array(
        'default'           => 30,
        'sanitize_callback' => 'absint',
    ));

    $wp_customize->add_control('muhtawaa_summary_length', array(
        'label'       => __('طول الملخص (كلمة)', 'muhtawaa'),
        'section'     => 'muhtawaa_content',
        'type'        => 'number',
        'input_attrs' => array(
            'min'  => 10,
    ...
    // عدد المقالات المشابهة
    $wp_customize->add_setting('muhtawaa_related_posts_count', array(
        'default'           => 3,
        'sanitize_callback' => 'absint',
    ));

    $wp_customize->add_control('muhtawaa_related_posts_count', array(
        'label'       => __('عدد المقالات المشابهة', 'muhtawaa'),
        'section'     => 'muhtawaa_content',
        'type'        => 'number',
        'input_attrs' => array(
            'min'  => 2,
