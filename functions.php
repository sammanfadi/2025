...
function muhtawaa_enqueue_assets() {
    ...
    wp_localize_script('muhtawaa-main', 'muhtawaa_settings', array(
        ...
        'showBackToTop'       => get_theme_mod('muhtawaa_show_back_to_top', true),
...
function muhtawaa_get_post_summary($post_id = null, $length = null) {
    ...
    if (!$length) {
        $length = absint(get_theme_mod('muhtawaa_summary_length', 30));
    }
...
function muhtawaa_display_post_summary() {
    if (!get_theme_mod('muhtawaa_show_summary', true)) {
        return;
    }
    $summary = muhtawaa_get_post_summary();
    if ($summary) {
